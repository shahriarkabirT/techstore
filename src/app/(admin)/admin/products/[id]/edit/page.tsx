...
import { useGetProductBySlugQuery, useUpdateProductMutation } from '@/redux/features/product/productApi';
...
export default function EditProductPage({ params }: PageProps) {
    const { id } = use(params);
    const { data: product, isLoading: isFetchingProduct } = useGetProductBySlugQuery(id);
...
function ProductForm({ product, id }: { product: any; id: string }) {
    const router = useRouter();
    const [updateProduct, { isLoading: isUpdating }] = useUpdateProductMutation();

    const [formData, setFormData] = useState({
        title: product.title || '',
        productType: (product.variants?.length > 0) ? 'variant' : (product.productType || 'single'),
        mrp: (product as any).mrp ?? '',
        price: (product as any).price ?? '',
        discountType: (product as any).discountType || 'percentage',
        discountValue: (product as any).discountValue || (product as any).discount || '',
        ...
        childCategory: (product as any).childCategory?._id || (product as any).childCategory || '',
        subChildCategory: (product as any).subChildCategory?._id || (product as any).subChildCategory || '',
        shortDescription: product.shortDescription || '',
        fullDescription: product.fullDescription || '',
        sizeGuide: (typeof product.sizeGuide === 'object' ? (product.sizeGuide as any)?.image : product.sizeGuide) || '',
    ...
    const handleInputChange = (e: any) => {
        ...
        setFormData((prev) => {
            ...
            if (name === 'mrp') {
                ...
                const mrp = Number(next.mrp) || 0;
                const discountValue = Number(name === 'discountValue' ? val : next.discountValue) || 0;
                const discountType = name === 'discountType' ? val : next.discountType;
    ...
    const handleSubmit = async (e: any) => {
        ...
        try {
                ...
                body: {
                    ...
                    productCost:
                        formData.productCost === '' || formData.productCost == null
                            ? null
                            : (() => {
                                  const n = Number(formData.productCost);
            ...
            if (res.success) {
                showSuccess('Product Updated', `${formData.title} has been saved successfully`);
        ...
        <div className="max-w-[1200px] mx-auto space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-black text-gray-900 tracking-tight uppercase">Edit Product</h1>
                        <p className="text-[10px] font-bold text-gray-600 font-mono uppercase tracking-widest mt-0.5">{id}</p>
                    </div>
                </div>

                <div className="flex items-center justify-between sm:justify-end gap-4 sm:gap-6 w-full sm:w-auto border-t sm:border-t-0 pt-4 sm:pt-0">
                    <label className="flex items-center gap-3 cursor-pointer group">
                        <span className="text-[10px] font-semibold uppercase tracking-widest text-gray-900">Published</span>
                    ...
                    <div className="flex gap-3">
                        ...
                        <button
                            ...
                            disabled={isUpdating}
                            className="px-6 py-2.5 bg-gray-900 text-white rounded-xl text-xs font-bold uppercase tracking-widest hover:bg-black transition-all active:scale-95 disabled:opacity-50"
                        >

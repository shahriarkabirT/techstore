...
import { useAdminUpdateReviewMutation } from '@/redux/features/reviews/reviewApi';
...
export default function EditReviewModal({ review, onClose }: EditReviewModalProps) {
    const [updateReview, { isLoading: isUpdating }] = useAdminUpdateReviewMutation();

    const isCustomerReview = !!review.userId;

    const [reviewerName, setReviewerName] = useState(review.reviewerName || review.userId?.name || '');
    const [reviewerAvatar, setReviewerAvatar] = useState(review.reviewerAvatar || review.userId?.image || '');
    const [rating, setRating] = useState(review.rating || 5);
    const [comment, setComment] = useState(review.comment || '');
    const [images, setImages] = useState<string[]>(review.images?.filter(Boolean) || []);
    
    const [isUploadingAvatar, setIsUploadingAvatar] = useState(false);
    const [isUploadingImages, setIsUploadingImages] = useState(false);

    const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        if (isCustomerReview) return;
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!isCustomerReview && !reviewerName.trim()) {
            return toast.error('Reviewer name is required');
        }
        ...
        try {
            await updateReview({
                ...
                comment,
                images,
                ...(isCustomerReview ? {} : { reviewerName, reviewerAvatar })
            }).unwrap();
            ...
            <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-6">
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="w-full px-4 py-2 border border-gray-200 rounded-lg bg-gray-50 text-gray-500 cursor-not-allowed">
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <input
                                    ...
                                    disabled={isCustomerReview}
                                    className={`w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 ${isCustomerReview ? 'bg-gray-50 text-gray-500 cursor-not-allowed' : ''}`}
                                />
                                {isCustomerReview && (
                                    <p className="text-xs text-gray-400 mt-1">Cannot edit real customer name</p>
                                )}
                            </div>
                            ...
                            <div>
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <label className={`border border-gray-200 px-4 py-2 rounded-lg text-sm font-medium transition-colors flex items-center gap-2 ${isCustomerReview ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : 'cursor-pointer bg-gray-50 hover:bg-gray-100 text-gray-700'}`}>
                                        <Upload className="w-4 h-4" />
                                        {isUploadingAvatar ? 'Uploading...' : 'Upload Avatar'}
                                        <input type="file" className="hidden" accept="image/*" onChange={handleAvatarUpload} disabled={isUploadingAvatar || isCustomerReview} />
                                    </label>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="pt-6 border-t border-gray-100 flex justify-end gap-3">
                        ...
                        <button
                            type="submit"
                            disabled={isUpdating || isUploadingAvatar || isUploadingImages}
